...
func (a *Agent) cachePeer(peer *Peer) error {
	// don't add self
	if peer.ID == a.config.NodeID {
		return nil
	}
	...
	if err := a.db.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists([]byte(dsLocalPeerBucketName))
		if err != nil {
			return err
		}
		if err := b.Put([]byte(peer.ID), peerData); err != nil {
			return err
		}
		return nil
	}); err != nil {
...
func getPeersFromCache(db *bolt.DB, seedPeers []string) ([]string, error) {
	logrus.Debugf("getPeersFromCache: %+v", seedPeers)
	var data [][]byte
	if err := db.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(dsLocalPeerBucketName))
		if b == nil {
			return nil
		}
		c := b.Cursor()
	...
	for _, p := range data {
		var peer *Peer
		if err := json.Unmarshal(p, &peer); err != nil {
			return nil, err
		}
